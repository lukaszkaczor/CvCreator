...
export class LanguagesFormComponent extends FormModel implements OnInit {
  ...
  constructor(builder: FormBuilder) {
    super(new StorageHelper('languageList'));
    this.index = -1;
  ...
  onSubmit(event) {
    ...
    if (!this.languageIsInTheList())
      super.onSubmit(event, { language: this.language.value, level: this.level.value } as ILanguage)
  }


  public languageIsInTheList() {
    for (let index in this.objectList)
      if (this.language.value != null && this.objectList[index].language.toLowerCase() == this.language.value.toLowerCase())
        return true;
