...
export class ImageFormComponent implements OnInit {
  ...
  constructor() {
    this._storageHelper = new StorageHelper('personalImage');
  }
  ...
  ngOnInit() {
    ...
    this._preview.src = this.imgIsNull ? this.defaultImage : this._storageHelper.get();
  }

  async readFile() {
    this.file = (<HTMLInputElement>document.querySelector('#fileInput')).files[0];
